...
def get_trace_target(device_name, termination_name):
    ...
    if front_port is not None:
        rear_port = getattr(front_port, "rear_port", None)
        rear_port_id = None

        if rear_port is not None:
            rear_port_id = (
                rear_port.get("id")
                if isinstance(rear_port, dict)
                else getattr(rear_port, "id", None)
            )
...
def get_target_trace(target):
    ...
    try:
        ...
        if (
            target["endpoint"] == "front-ports"
            and response is not None
            and response.status_code == 404
        ):
            rear_port_id = target.get("rear_port_id")

            if rear_port_id is None:
                raise ValueError(
                    "Le front port n'a pas de rear port associé exploitable "
                    "pour lancer le trace path."
                ) from error
